...
# Load the data model from MongoDB
...
def get_hierarchy():
    hierarchy = build_parent_child_hierarchy()
...
def filter_hierarchy(mondo_id):
    ...
    if not diseases_collection.find_one({"id": full_id}):
        abort(404, description="Disease not found")
    hierarchy = filter_hierarchy_by_mondo_id(full_id)
    return jsonify(hierarchy)
...
def diseases_by_phenotypes():
    ...
    diseases = get_diseases_by_phenotypes(full_phenotype_ids, data_model)
    diseases = convert_objectid_to_str(diseases)
...
def diseases_by_age_onsets():
    ...
    diseases = get_diseases_by_age_onsets(full_age_onset_ids, data_model)
    diseases = convert_objectid_to_str(diseases)
    print(f"Found diseases: {diseases}")
    return jsonify(diseases)
...
def diseases_by_anatomical_structures():
    ...
    diseases = get_diseases_by_anatomical_structures(full_anatomical_ids, data_model)
    diseases = convert_objectid_to_str(diseases)
    print(f"Found diseases: {diseases}")
    return jsonify(diseases)
...
def diseases_by_filters():
    ...
    diseases = get_diseases_by_filters(full_phenotype_ids, full_anatomical_ids, full_age_onset_ids, data_model)
    diseases = convert_objectid_to_str(diseases)
    return jsonify(diseases)
